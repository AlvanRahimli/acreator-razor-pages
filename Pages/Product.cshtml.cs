using System;
using System.Collections.Generic;
using System.Linq;
using System.Net.Http;
using System.Threading.Tasks;
using acreator_front.Models;
using Microsoft.AspNetCore.Mvc;
using Microsoft.AspNetCore.Mvc.RazorPages;
using Newtonsoft.Json;

namespace acreator_front.Pages
{
    public class ProductModel : PageModel
    {
        private readonly IHttpClientFactory _clientFactory;

        public int Id { get; set; }

        public ProductModel(IHttpClientFactory clientFactory)
        {
            this._clientFactory = clientFactory;
        }

        public async Task OnGetAsync(int id)
        {
            Id = id;

            var client = _clientFactory.CreateClient("getProductClient");

            var request = new HttpRequestMessage(HttpMethod.Get, $"http://localhost:5000/products/{id}");
            request.Headers.Add("Accept", "application/json");

            var response = await client.SendAsync(request);

            if (response.IsSuccessStatusCode)
            {
                var data = await response.Content.ReadAsStringAsync();
                var serialized = JsonConvert.DeserializeObject<ProductResponseModel>(data);

                ViewData["product"] = serialized.Data;
            }
            else
            {
                RedirectToPage("products");
            }
        }
    }
}
